...
// Network topology      (TCP2)   
//                       (UDP)                 (UDP)
//                        n5                    n6 (enable/disable)
//                        |                     |
//                        |  2 Mbps             |  2 Mbps
//                        |  10 ms              |  10 ms         n7 (TCP2 Sink)
//                        |                     |                |
...
// - Flow from n5 to n4 using BulkSendApplication.
...
int main(int argc, char **argv)
{
    ...
    double      qw = 0.002;
    double      maxP = 10;
    string      dRate = "2Mbps";        //UDP source data rate
    string      dRate2 = "0Mbps";        //UDP source2 data rate
    string      RTT = "10ms";            //round trip time
    ...
    NodeContainer net7 (p2pNodes.Get(3), p2pNodes.Get(7));
    ...
    NetDeviceContainer nodeDevices7;
    nodeDevices7 = PTPNode37.Install(net7);
    ...
    p2pInterfaces7 = address6.Assign (nodeDevices7);
    ...
    BulkSendHelper source2 ("ns3::TcpSocketFactory",
                         InetSocketAddress (p2pInterfaces7.GetAddress (1), port));
     // Set the amount of data to send in bytes.  Zero is unlimited.
    source2.SetAttribute ("MaxBytes", UintegerValue (maxBytes));
    source2.SetAttribute ("SendSize", UintegerValue (pktSize));
    ApplicationContainer sourceApps2 = source.Install (p2pNodes.Get (0));
    ...
    PacketSinkHelper sink2 ("ns3::TcpSocketFactory",
                            InetSocketAddress (Ipv4Address::GetAny (), port));
    ApplicationContainer sinkApps2 = sink2.Install (p2pNodes.Get (7));
    ...
    clientHelper.SetAttribute("PacketSize", UintegerValue (pktSize));
    clientHelper2.SetConstantRate(y,pktSize);
    ...
    Ptr<PacketSink> sinkTCP = DynamicCast<PacketSink> (sinkApps.Get (0));    
    std::cout <<"MinTh: "<<minTh<<" MaxTh: "<<maxTh<<" MaxP: "<<maxP<<" RTT: "<<RTT<<" DataRate: "<<dRate<< " Goodput TCP: "<< (double)sinkTCP->GetTotalRx () / (double)10.0<< std::endl;

    Ptr<PacketSink> sinkTCP2 = DynamicCast<PacketSink> (sinkApps2.Get (0));    
    std::cout <<"MinTh: "<<minTh<<" MaxTh: "<<maxTh<<" MaxP: "<<maxP<<" RTT: "<<RTT<<" DataRate: "<<dRate<< " Goodput TCP2: "<< (double)sinkTCP2->GetTotalRx () / (double)10.0<< std::endl;

    Ptr<PacketSink> sinkUDP = DynamicCast<PacketSink> (apps.Get (0));    
    std::cout <<"MinTh: "<<minTh<<" MaxTh: "<<maxTh<<" MaxP: "<<maxP<<" RTT: "<<RTT<<" DataRate: "<<dRate<< " Goodput UDP: "<< (double)sinkUDP->GetTotalRx () / (double)10.0<< std::endl;
