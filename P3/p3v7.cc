...
int main(int argc, char **argv){
	  ...
	  uint32_t      nodes 					= 20;  				    	//the number of WiFi nodes
    ...
    //
  	...
  	double 		  dRateDec 					= 54.0 * trafficInt/nodes;			//calculating data rate (54Mbps x intensity/nodes)
  	...
  	clientHelper.SetAttribute ("OnTime", StringValue ("ns3::ExponentialRandomVariable[Min=0.,Max=1.]"));
  	clientHelper.SetAttribute ("OffTime", StringValue ("ns3::ExponentialRandomVariable[Min=0.,Max=0.]"));
    ...
    randomAssign(nodes);
  	...
  	cout<<"Output\t"<<nodes<<"\t"<<Ptx<<"\t"<<trafficInt<<"\t"<<rtalgo.c_str()<<"\t"<<(float) totalRx<<endl;  
