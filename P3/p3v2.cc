...
void randomAssign(uint32_t nodes){
    ...
    cout<<"mid 0: "<<mid[0]<<endl;
    cout<<"sources 0: "<<sources[0]<<endl;
    cout<<"sources 1: "<<sources[1]<<endl;
...
int main(int argc, char **argv){
	  ...
	  uint32_t      nodes 					= 25;  				    	//the number of WiFi nodes
	...
	DataRate x(dRate);
  	...
  	clientHelper.SetAttribute ("OffTime", StringValue ("ns3::UniformRandomVariable[Min=0.,Max=0.]"));
    ...
    randomAssign(nodes);
  	...
  	printf("MyOutput\t%d\t%lf\t%lf\t%s\t%f\n",nodes,Ptx,trafficInt,rtalgo.c_str(),(float) totalRx);
